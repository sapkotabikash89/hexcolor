...
interface ColorLibraryClientProps {
  initialCategory?: string
  page?: number
  hidePagination?: boolean
}

export function ColorLibraryClient({
  initialCategory = "all",
  page = 1,
  hidePagination = false,
}: ColorLibraryClientProps) {
  ...
  const allColors: ColorItem[] = colorLibraryData
  ...
  const filteredColors = () => {
    if (!searchQuery) {
      ...
      return allColors.filter((c) => (c as any).category === currentCategory)
    }
                  ...
                  <div className="divide-y">
                    {previewResults.map((c, i) => (
                      ...
                      >
                        ...
                        <div className="flex-1 text-sm">{highlight(c.name, searchQuery.trim())}</div>
      ...
      <div className="w-full">
        ...
        {!searchQuery && currentCategory === "all" ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {colorsToShow.slice(0, visibleCount).map((color, index) => (
                <LibraryColorSwatch key={index} name={color.name} hex={color.hex} />
              ))}
            </div>
            {visibleCount < colorsToShow.length && (
